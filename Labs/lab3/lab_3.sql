--1 EX
--A
SELECT course_id,title,credits FROM course WHERE credits > 3;
--B
...
WHERE section.semester = 'Fall' AND course.course_id = section.course_id;
--E
...
--B
SELECT department.building FROM department,course
    WHERE department.dept_name = course.dept_name
    GROUP BY department.building
    HAVING COUNT(course_id) =
           (SELECT MAX(amount) FROM (SELECT COUNT(course_id) amount FROM department, course
                WHERE course.dept_name = department.dept_name
                GROUP BY department.building) AS temp);

--C
SELECT temp.d_n FROM (SELECT COUNT(dept_name) AS count,dept_name AS d_n FROM course GROUP BY dept_name) AS temp
WHERE temp.count = (SELECT MIN(count) FROM (SELECT COUNT(dept_name) AS count FROM course GROUP BY dept_name) AS min);
--OR--Second way using "HAVING"
SELECT course.dept_name FROM course GROUP BY course.dept_name
HAVING COUNT(dept_name) = (SELECT MIN(count) FROM (SELECT COUNT(dept_name) AS count FROM course GROUP BY dept_name) AS m);
...
SELECT instructor.ID,instructor.name FROM instructor,teaches
    WHERE teaches.ID = instructor.ID AND teaches.year = 2018
EXCEPT
    (SELECT instructor.ID, instructor.name FROM instructor,teaches
...
--3 EX
--A
SELECT student.name,student.ID FROM student,takes,course
...
--B
SELECT instructor.name, advisor.i_id FROM instructor,advisor,student,takes
WHERE instructor.ID = advisor.i_ID
      AND student.ID = advisor.s_ID
      AND takes.grade IN ('B-','C','C+','C-','D+','D','D-','F')
